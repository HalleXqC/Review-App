...
const Movies = () => {
        ...
        .then(res => { 
            const data = Object.entries(res).map(item => {
                ...
                if(item[1].reviews){
                    ...
                    const averageRating = (allReviews.reduce((total, current) => +total + +current[1].rating, 0)) / allReviews.length
                    return {
                        ...item[1],
                        id,
                        averageRating
    ...
    return (
        <div className={cls.root}>
            ...
            {movies && sort && typeof showSort === 'boolean' && layout ? (
                <>
                    <div className={cls.main}>
                        <div className={cls.mainSettings}>
                                        ...
                                        {[...Array(3)].map((item, i) => {
                                            ...
                                            return (
                                                <button 
                                                    ...
                                                    type="button"
                                                    onClick={() => {setSort(`${sortOptions[i]}`)}}
                                                    disabled={sort === sortOptions[i] ? true : false}
                                                    style={sort === sortOptions[i] ? {background: "linear-gradient(to right, rgba(35,0,212,1), rgb(177, 115, 207))"} : null}
                                                    key={i}
                                                >
                                                    {sortOptions[i]}
                                                </button>
                            ...
                            <div className={cls.setsRight}>
                                <button 
                                    ...
                                    style={layout === 'regular' ? {color: "crimson"} : {color: "#eee"}} 
                                >
                                    <RegularLayout/>
                                </button>
                                <button 
                                    ...
                                    style={layout === 'alt' ? {color: "crimson"} : {color: "#eee"}} 
                                >
                                    <AltLayout/>
                                </button>
                            </div>
                        ...
                        <div className={cls.mainCards}>
                            {
                                sort === 'Alphabet' ? (
                                    movies.sort((a, b) => a.title.trim() > b.title.trim() ? 1 : -1).map((item , i) => (
                                        <CardMovie key={i} item={item} layout={layout} push={pushPage}/>
                                    ))
                                ) : sort === 'Genres' ? (
                                    movies.sort((a, b) => a.genres.trim().split(' ')[0] > b.genres.split(' ')[0] ? 1 : a.genres.trim().split(' ')[0] < b.genres.split(' ')[0] ? -1 : 0).map((item , i) => (
                                        <CardMovie key={i} item={item} layout={layout} push={pushPage}/>
                                    ))
                                ) : sort === 'Rating' ? (
                                    movies.sort((a, b) => a.averageRating > b.averageRating ? 1 : a.averageRating < b.averageRating ? -1 : 0).reverse().map((item , i) => (
                                        <CardMovie key={i} item={item} layout={layout} push={pushPage}/>
                                    ))
                                ) : <h1>Problem</h1>
                            }
